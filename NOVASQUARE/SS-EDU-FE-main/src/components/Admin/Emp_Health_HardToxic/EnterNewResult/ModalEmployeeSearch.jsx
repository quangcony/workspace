...
const ModalSearchNV = ({ isOpen, onCancel, onAddNew }) => {
  ...
  useEffect(() => {
    if (
      filterEmployee?.FIRST_NAME ||
      filterEmployee?.LAST_NAME ||
      filterEmployee?.CD ||
      filterEmployee?.BOD ||
      ...
      if (filterEmployee?.FIRST_NAME) {
        let tempDatas = [];
        checkFilterList(newDatas, employeeList).forEach((item, index) => {
          const dataOfList = item.User?.FIRST_NAME.toLowerCase().trim();
      ...
      // FILTER BY LAST_NAME
      if (filterEmployee.LAST_NAME) {
        const newQuery = removeAccents(String(filterEmployee.LAST_NAME))
          .toLowerCase()
          .trim();
        const tempData = checkFilterList(newDatas, employeeList).filter(
          (item) =>
            removeAccents(String(item?.User?.LAST_NAME))
              .toLowerCase()
              .trim()
              .includes(newQuery)
        );
      ...
      if (filterEmployee?.BOD) {
        let tempDatas = [];
        checkFilterList(newDatas, employeeList).forEach((item, index) => {
          const dataOfList = moment(new Date(item.User?.BOD)).format(formatDate.Type);
      ...
      if (filterEmployee?.START_WORKING_DATE) {
        let tempDatas = [];
        checkFilterList(newDatas, employeeList).forEach((item, index) => {
          const dataOfList = moment(new Date(item.START_WORKING_DATE)).format(formatDate.Type);
          const keyWord = moment(filterEmployee.START_WORKING_DATE).format(formatDate.Type);
          const isMatching = dataOfList === keyWord;
          if (isMatching) {
            tempDatas.push(item);
          }
      ...
      if (filterEmployee?.PHYSICAL_DATE) {
        let tempDatas = [];
        checkFilterList(newDatas, employeeList).forEach((item, index) => {
          const dataOfList = moment(new Date(item.PHYSICAL_DATE)).format(formatDate.Type);
